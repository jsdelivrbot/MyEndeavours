...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 10.1.3.4.0(build 080718.0645) AT [MON MAY 25 21:16:37 IST 2009]. -->
?>
<xsl:stylesheet version="1.0"
                ...
                exclude-result-prefixes="xsl ns7 plt ns3 ns0 jca ns1 ns8 ns10 ns2 pc ns9 ns6 ns4 ns5 hdr imp1 tns ehdr xp20 ora orcl bpws eth hwf xref ids">
  <xsl:template match="/">
    <imp1:RootElement>
      <ScMasterTransaction>
        <Message_ID>
          <xsl:value-of select="/ns2:CustomChangePO/Message_id"/>
        </Message_ID>
        <xsl:if test="/ns2:CustomChangePO/Attribute5 ='TP1'">  
            <flowId3>
      ...
      <scDetailedTransaction>
        ...
        <payload>
          <xsl:value-of select="orcl:get-content-as-string(/ns2:CustomChangePO/ChangePO)"/>
        </payload>
        <businessDocumentID>
          <xsl:value-of select="/ns2:CustomChangePO/Sender_Document_number"/>
        </businessDocumentID>
        <source>
          <xsl:text disable-output-escaping="no">ETH</xsl:text>
        </source>
        <xsl:choose>
          <xsl:when test='/ns2:CustomChangePO/Attribute5 = "TP1"'>
            <destinationSystem>
              <xsl:value-of select="/ns2:CustomChangePO/Trading_partner_1_name"/>
            </destinationSystem>
          </xsl:when>
          <xsl:when test='/ns2:CustomChangePO/Attribute5 = "TP2"'>
            <destinationSystem>
              <xsl:value-of select="/ns2:CustomChangePO/Trading_partner_2_name"/>
            </destinationSystem>
          </xsl:when>
          <xsl:when test='/ns2:CustomChangePO/Attribute5 = "TP3"'>
            <destinationSystem>
              <xsl:value-of select="/ns2:CustomChangePO/Trading_partner_3_name"/>
            </destinationSystem>
          </xsl:when>
          <xsl:when test='/ns2:CustomChangePO/Attribute5 = "TP4"'>
            <destinationSystem>
              <xsl:value-of select="/ns2:CustomChangePO/Logistics_1_name"/>
            </destinationSystem>
          </xsl:when>
          <xsl:when test='/ns2:CustomChangePO/Attribute5 = "TP5"'>
            <destinationSystem>
              <xsl:value-of select="/ns2:CustomChangePO/Logistics_2_name"/>
            </destinationSystem>
          </xsl:when>
          <xsl:otherwise>
            <destinationSystem>
